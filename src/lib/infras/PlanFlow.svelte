<script lang="ts">
	...
	const goToCheckout = async () => {
		...
		const orderProducts = buildSubscriptionProducts();
		if (
			orderProducts.some((product) => !product.productId || product.productId.includes('REPLACE'))
		) {
			checkoutError =
...
>
	<div
		class="relative max-h-[92vh] w-full max-w-6xl overflow-y-auto rounded-[22px] border border-white/20 bg-[linear-gradient(180deg,rgba(255,255,255,0.97),rgba(255,249,245,0.95))] p-3 shadow-[0_28px_80px_rgba(38,16,31,0.24)] sm:max-h-[90vh] sm:w-[96%] sm:rounded-[28px] sm:border-white/40 sm:p-6 lg:w-[94%] lg:p-8"
	>
		<button
		...
		{#if step === 1}
			...
			<div
				class="h-56 overflow-y-auto rounded-[20px] border border-[#e4d9c8] bg-white/80 p-4 text-sm leading-7 text-[#5f4b53]"
			>
				<p>
					...
					/>
					2. ベーシックプランの月額利用料は1台あたり3,300円（税込）とします。<br />
					<br />
					...
					<br />
					第4条（解約および返却義務）<br />
					1. 解約を希望する場合は、当社指定のメールアドレス（tft.kao@gmail.com）または電話番号（03-3818-0408）宛にお問い合わせいただくものとします。<br
					/>
			...
			<div class="mt-5 flex items-center rounded-2xl bg-[#f8f4ec] px-4 py-3">
				<input
					...
					type="checkbox"
					bind:checked={agreed}
					class="mr-3 h-4 w-4 accent-[#8a6c57]"
				/>
				<label for="agree" class="text-sm text-[#5f4b53]"
					>利用規約に同意してプラン選択へ進みます</label
				>
			</div>
		...
		{:else if step === 2}
			<div class="mb-4 overflow-hidden bg-transparent sm:mb-6">
				<div
					class="rounded-[18px] border border-[#e4d9c8]/70 bg-[#f7f3ed] p-4 sm:rounded-[20px] sm:p-5"
				>
					<p class="mt-2 text-sm leading-6 text-[#5f4b53]">お申し込み内容をご確認ください。</p>
				</div>
				<div class="bg-transparent p-0 pt-4">
					<div class="grid gap-3 md:gap-4">
						<div
							class="rounded-[18px] border border-[#e4d9c8]/70 bg-white/92 p-3 shadow-[0_8px_20px_rgba(77,63,39,0.04)] sm:rounded-[20px] sm:p-4 md:shadow-none"
						>
							<div class="overflow-hidden rounded-[16px] bg-[#f3ede3] sm:rounded-[20px]">
								{#if plan.image.src}
									<img
										src={plan.image.src}
										alt={plan.image.alt}
										class="h-40 w-full bg-[#f3ede3] object-contain p-2 sm:h-48 md:h-40 lg:h-48"
									/>
								{:else}
									<div
										class="flex h-40 w-full items-center justify-center bg-[#f3ede3] text-center text-[#867c6c] sm:h-48 md:h-40 lg:h-48"
									>
										<div>
							...
							<div class="pb-2">
								<div class="mt-3 text-sm font-semibold text-[#8a6c57] sm:mt-4">
									{plan.priceLabel}
								</div>
							</div>
							...
							<button
								class="mt-4 w-full rounded-full border border-[#d8cdbb] bg-[#f8f4ec] px-4 py-2.5 text-sm text-[#2e1d24] transition hover:bg-[#efe6d9]"
								on:click={next}
							>
								このプランで申し込む
							</button>
		...
		{:else}
			<div class="grid gap-4 md:grid-cols-[1.05fr_0.95fr] sm:gap-5">
				<div
					class="overflow-hidden rounded-[20px] border border-[#e4d9c8]/70 bg-white shadow-[0_12px_28px_rgba(77,63,39,0.06)] sm:rounded-[28px] sm:border-[#e4d9c8] sm:shadow-[0_18px_40px_rgba(77,63,39,0.08)]"
				>
					<div class="overflow-hidden rounded-t-[20px] bg-[#f3ede3] sm:rounded-t-[28px]">
						{#if plan.image.src}
							<img
								src={plan.image.src}
								class="h-52 w-full bg-[#f3ede3] object-contain p-2"
								alt={plan.image.alt}
							/>
						{:else}
				...
				<div
					class="rounded-[20px] border border-[#e4d9c8]/70 bg-[linear-gradient(180deg,#fffdfb,#f8f4ec)] p-3 shadow-[0_12px_28px_rgba(77,63,39,0.06)] sm:rounded-[28px] sm:border-[#e4d9c8] sm:p-5 sm:shadow-[0_18px_40px_rgba(77,63,39,0.08)]"
				>
					<h3 class="mt-2 text-xl text-[#2e1d24] sm:text-2xl">ご注文内容の確認</h3>

					<div class="mt-4 flex flex-col gap-3">
						<button
							class="w-full rounded-full border border-[#d8cdbb] bg-[#f8f4ec] px-5 py-3 text-sm text-[#2e1d24] transition hover:bg-[#efe6d9] disabled:opacity-50"
						...
						<button
							class="w-full rounded-full border border-[#d8cdbb] px-5 py-3 text-sm text-[#5f4b53] bg-white/80 transition hover:bg-[#f8f4ec]"
							on:click={back}
							disabled={isProcessing}
						>
							プラン選択に戻る
					...
					<div
						class="mt-4 rounded-[18px] border border-[#d8cdbb] bg-[#efe8dc] p-4 text-[#2e1d24] sm:mt-5 sm:rounded-[22px] sm:p-5"
					>
						<div class="text-sm text-[#776959]">毎月のお支払い</div>
						<div class="mt-2 text-4xl">{formatYen(plan.price)}</div>
